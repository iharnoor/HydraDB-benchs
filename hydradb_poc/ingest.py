"""
Ingest the relationship timeline into both HydraDB and Mem0.

HydraDB: uses /ingestion/upload_knowledge (PDF upload)
Mem0:    uses /v1/memories/ (chunked text, since Mem0 doesn't support PDF upload)

We also extract the text from the PDF so Mem0 gets the same content.
"""
...
# ── Text chunks matching the PDF content ─────────────────────
# These mirror the enriched descriptions in the PDF so Mem0 gets
# the same information that HydraDB extracts from the PDF.
...
def ingest_hydradb(client: HydraDBClient) -> list[dict]:
    """Ingest timeline chunks into HydraDB as memories."""
    print(f"\n  [HydraDB] Ingesting {len(TIMELINE_CHUNKS)} memory chunks...")
    memories = [{"text": chunk} for chunk in TIMELINE_CHUNKS]

    # Batch ingest all memories at once
    result = client.add_memories(memories)
    print(f"  [HydraDB] Ingestion response: {result}")

    # Brief pause for indexing
    print("  [HydraDB] Waiting 10s for indexing...")
    time.sleep(10)

    return result


def ingest_mem0(client: Mem0Client) -> list[dict]:
    """Ingest timeline chunks into Mem0 as memories."""
    print(f"\n  [Mem0] Ingesting {len(TIMELINE_CHUNKS)} memory chunks...")
    results = []
    for i, chunk in enumerate(TIMELINE_CHUNKS):
        try:
            result = client.add_memory(chunk, metadata={"source": "relationship_timeline"})
            results.append(result)
            print(f"  [Mem0] Chunk {i+1}/{len(TIMELINE_CHUNKS)} ingested")
        except Exception as e:
            print(f"  [Mem0] Chunk {i+1} failed: {e}")
            results.append({"error": str(e)})
        time.sleep(0.5)  # rate limit
    print(f"  [Mem0] Done! {len(results)} chunks ingested.")
    return results
...
def main():
    ...
    if not hydradb_key:
        print("WARNING: HYDRADB_API_KEY not set, skipping HydraDB ingestion")
    if not mem0_key:
        print("WARNING: MEM0_API_KEY not set, skipping Mem0 ingestion")

    # HydraDB ingestion
    if hydradb_key:
        hydra = HydraDBClient(api_key=hydradb_key)
        tenant_id = "live_benchmark"
        hydra.use_tenant(tenant_id)
        print(f"  [HydraDB] Using tenant: {tenant_id}")
        try:
            ingest_hydradb(hydra)
        except Exception as e:
            print(f"  [HydraDB] Ingestion failed: {e}")
